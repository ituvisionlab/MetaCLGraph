...
class GCN(nn.Module):
    ...
    def maml_batch(self, blocks, features, params):
        e_list = []
        h = features
        for i,(layer, fast) in enumerate(zip(self.gat_layers[:-1], params[:-1])):
            h, e = layer.maml_batch(blocks[i], h, fast)
            h = F.relu(h)
            e_list = e_list + e
            h = F.dropout(h, p=self.dropout, training=self.training)
        logits, e = self.gat_layers[-1].maml_batch(blocks[-1], h, params[-1])
        self.second_last_h = logits if len(self.gat_layers) == 1 else h
        e_list = e_list + e
        return logits, e_list
